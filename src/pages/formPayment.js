...
function Payment() {
    ...
    const [message, setMessage] = useState('Va rog sa completati urmatoarele campuri pentru a trimite emailul: \n' +
                                            '\n' + 'Nume: \n'  + '\n' + 'Prenume: \n'  + '\n' + 'Email: \n'  + '\n' + 'Categoria Juridica: \n'  + '\n'  + '\n' + 'Mesaj:'
                                            + '\n' + '\n' + '\n' + '\n' + '\n' + 'Va rog sa atasati fisierele necesare in sectiunea ATASAMENTE');
    const [subject, setSubject] = useState('Consult Juridic');

    const handleButtonClick = () => {
        const mailtoLink = `mailto:${sender}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`;
